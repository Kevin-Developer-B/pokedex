...
<header>
    <div class="headline">
        ...
        <div>
            <input type="search" id="pokemon-search" placeholder="Search Pokémon" oninput="filter()"/>
        </div>
    </div>
</header>
...
return `
    <div class="cards">
        <div class="index-name">
            <p>#${index + 1}</p> <p>${pokemonRenderList[index].pokemonData.name.charAt(0).toUpperCase() + pokemonRenderList[index].pokemonData.name.slice(1)}</p>
...
return`
    <div class="overlay-container" onclick="event.stopPropagation()">
        <div class="overlay-pokename-position">
           <p>#${i+1}</p> <p>${pokemonRenderList[i].pokemonData.name.charAt(0).toUpperCase() + pokemonRenderList[i].pokemonData.name.slice(1)}</p>
        </div>
